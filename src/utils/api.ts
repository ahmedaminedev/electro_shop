...
export const api = {
    ...
    logout: async () => {
        try {
            await fetch(`${API_URL}/auth/logout`, { headers: getAuthHeaders() });
    ...
    getBlogPosts: async () => {
        ...
        return handleResponse(res, 'Erreur chargement blog');
    },
    getBlogPostBySlug: async (slug: string) => {
        const res = await fetch(`${API_URL}/blog/${slug}`);
        return handleResponse(res, 'Erreur chargement article');
    },
    ...
    sendMessage: async (data: { name: string; email: string; subject: string; message: string }) => {
        ...
        return handleResponse(res, 'Erreur envoi message');
    },
    getMessages: async () => {
        const res = await fetch(`${API_URL}/contact`, {
            headers: getAuthHeaders(),
        });
        return handleResponse(res, 'Erreur chargement messages');
    }
};
